...
# The committed board on the 4-layer stackup with the two pinned edge rows.
# Found by find_board.py over 24 seeds and then a route-order sweep.  A
# bare
...
def _expanded_boxes(feats):
    ...
    # Ensure contiguous arrays for fast vectorised ops and safe C interop
    return (np.ascontiguousarray(x0), np.ascontiguousarray(y0),
            np.ascontiguousarray(x1), np.ascontiguousarray(y1),
            np.ascontiguousarray(lay))
