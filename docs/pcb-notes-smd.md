...
`tools/gen_pcb_smd.py`. **76.2 × 50.8 mm**, two layers, fully routed,
with the frequency pots mounted on the board and trace width matched to
purpose - 10 mil for signal, 16 mil for +15V/-15V/GND.
...
```
board 76.2 x 50.8 mm | 46 footprints | 123 pads | 114 tracks | 46 vias
board utilisation 60%; largest empty rectangle 10 x 25 mm
...
* **Clearance** — every pair of copper features (pad, track segment, via) that
  ...
  using each net's *actual* trace width (10 mil signal, 16 mil power/ground -
...
left in disconnected pieces; and, when trace widths were first split by
purpose, pad keepouts that had been sized from the *pad's own* net rather than
from the widest trace that might pass nearby - backwards, and the cause of
every clearance violation that turned up while that version was in place.
...
| | Width | Carries |
|---|---|---|
| Signal | 10 mil | A few mA at most - the crossover's actual audio signal |
| +15V, -15V, GND | 16 mil | The combined supply current of both quad op-amps, and (GND) the audio return path |

The wider supply/ground traces are why the board grew from 76.2 × 44.5 mm to
76.2 × 50.8 mm - about 14 % more area for meaningfully more robust power
delivery and a trace that visibly reads as a rail rather than a signal line.
...
| **…plus width matched to purpose** | **76.2 × 50.8 mm** | **46** |

The last row is placement, not routing: the slot grid used to stop well above
the potentiometers, so the strips either side of them and the gap between them
were dead board. Letting slots run to the bottom edge — the pot courtyards
already exclude their own footprints — took 27 % off the height.

Fewer vias as well as less area — on a signal board that is worth having in its
own right. `IC_ROT=0` and `IC_LAYOUT=stacked` select the earlier arrangements.
...
pairwise swaps scored on ratsnest length (801 → 749 mm).
