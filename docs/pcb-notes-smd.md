...
`tools/gen_pcb_smd.py`. **81.3 × 68.6 mm**, two layers, fully routed,
...
```
board 81.3 x 68.6 mm | 44 footprints | 121 pads | 108 tracks | 50 vias
...
**The two quads sit side by side**, so each filter occupies its own half of the
board with its frequency pot directly below it. This was tried and rejected at
320 × 260 units, where one ground pin (U1D.12) would not route; ten more units
of height fixes it, and the arrangement then routes *better* than stacked —
50 vias instead of 57, and a shorter ratsnest — for 2.5 mm of extra height.
`IC_LAYOUT=stacked` selects the old arrangement.
...
pairwise swaps scored on ratsnest length (944 → 867 mm).
