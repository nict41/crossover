...
`tools/gen_pcb_smd.py`. **146.3 × 74.7 mm**, two layers, with nine
front-panel controls on the board - a frequency pot and a volume trim for
each of the three bands, a master volume, and a mute button per band -
trace width matched to purpose (12 mil signal, 16 mil +15V/-15V/GND), and
45° chamfered corners throughout.

> **This board does not verify clean.** Eight nets are left in pieces -
> including both supply rails - and the ground pour misses three pads:
> 10 DRC problems. It is the best of ~115 placement seeds plus
> route-order sweeps. Adding the three
> panel mute buttons consumed the routing slack the two-layer board was
> living on; the levers tried and rejected, and the ones still worth
> trying, are in `CLAUDE.md` under "The board does NOT currently verify
> clean". The version before the buttons (114.3 × 77.5 mm, 83 footprints)
> did verify clean and passed `validate_fab.py --online`; it is in the git
> history.
...
DRC PROBLEMS (10):
  - net +15V is in 3 pieces
  - net -15V is in 2 pieces
  - net HIGH_MUTE is in 2 pieces
  - net MG2 is in 3 pieces
  - net MG3 is in 2 pieces
  - net N2150_1250 is in 2 pieces
  - net N2150_1550 is in 2 pieces
  - net N680_300 is in 2 pieces
  - a single empty rectangle is 10% of the board (24 x 47 mm at 0,5)
  - the ground pour does not reach 3 pad(s): Q3.2, U2B.5, U3D.12
```

The checks below all still run and all still mean what they say; what has
changed is that the board no longer passes them. The list above is the
output of the same `verify()` that used to print `verified: ...`.
