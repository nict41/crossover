...
`tools/gen_pcb_smd.py`. **114.3 × 77.5 mm**, two layers, fully routed,
with six front-panel controls on the board - a frequency pot and a volume
trim for each of the three bands, plus a master volume - trace width
matched to purpose (12 mil signal, 16 mil +15V/-15V/GND), and 45° chamfered
corners throughout.
...
board utilisation 60%; largest empty rectangle 24 x 29 mm at (0, 14)
...
(VR4) - HIGH/MID (VR1) - HIGH VOL (VR3)**, with **MASTER (VR6)** at the
left-hand end - six controls on a uniform
...
spacing - see "Routing more controls" below for what happened when the
...
* **J6** — the 8-pin header carrying the mute switches and LEDs to the
  front panel. DC only; no audio leaves the board on it.
...
* **The front-panel row** is a rigid group: six controls, one line, uniform
