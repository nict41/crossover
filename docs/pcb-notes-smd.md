...
`tools/gen_pcb_smd.py`. **73.2 × 48.5 mm**, two layers, fully routed,
with the frequency pots mounted on the board, trace width matched to
purpose - 12 mil for signal, 16 mil for +15V/-15V/GND - and 45° chamfered
corners throughout.
...
```
board 73.2 x 48.5 mm | 46 footprints | 123 pads | 122 tracks | 61 vias
board utilisation 75%; largest empty rectangle 6 x 25 mm at (0, 19)
...
physical validity so the search can never produce an overlap (933 → 846 mm
...
* **Board size is searched, not chosen.** `BOARD_W`, `BOARD_H` and
  ...
  smallest size that still routes *and* verifies. For the current arrangement
  (rotated, side-by-side, pot band used), 288 × 191 units (73.2 × 48.5 mm) is
  that floor - checked directly (287 wide and 190 tall both fail), not
  estimated. The stacked/unrotated alternatives above have their own,
  separately-established floors from an earlier round and have not been
  re-swept against the current trace widths and placement search.
* **Utilisation is 75 %, not 100 %, and that's expected.** On two layers, with
  12/16 mil track and 8 mil clearance and SOIC pads that can only break out
  sideways, the router is what sets the floor — packing the parts tighter
  makes nets unroutable long before the parts themselves collide, which is
  exactly what the 287/190-unit sweep failures above demonstrate directly.
  The remaining largest empty rectangle (6 × 25 mm) is routing headroom this
  arrangement needs, not an unreclaimed pocket — the reclaimable pocket that
  did exist (14 × 17 mm beside VR1) is what the courtyard-aware placement
  search above already took back.
* **On-board pots cost area.** Two 9 mm dual pots and their knob spacing take
  room the wiring pads did not. Both are inside JLCPCB's 100 × 100 mm price
  tier, so fabrication cost is unchanged; what you save is twelve pot wires.
...
* **Verify every hand-drawn footprint against its real datasheet before
  ...
  (pad spacing/diameter), and the pot's locating-boss hole diameter are all
  parametrised guesses at the standard pattern, not traced from a specific
  manufacturer's drawing. Cheap to get wrong, cheap to check.
