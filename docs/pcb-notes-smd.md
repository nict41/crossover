...
`tools/gen_pcb_smd.py`. **147.3 × 84.8 mm**, four layers, all 67 nets routed,
...
**One defect outstanding, and it is a placement one.** Every signal net
routes; five ground pads sit in a pocket the pour cannot enter. The same
five fail at all twelve route orders tried on this placement, which is the
signature `find_board.py` calls out: pour-reachability-only failures are
made by the placer, and route order cannot open them. A seed search is the
lever. Do not order this revision.

**The pinned rows cost height, and that is the honest price of the
constraint.** The board before them was 141.7 × 70.4 mm and verified clean,
but it did that partly by putting parts in the strip in front of the pot
row - where the front panel goes - and behind the connector row. Forbidding
that pushes the two rows apart: 70.4 mm becomes 84.8. The area is real and
so is the requirement; what has not yet been recovered is the clean
verify.
