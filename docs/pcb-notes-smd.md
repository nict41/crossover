...
`tools/gen_pcb_smd.py`. **147.3 × 68.3 mm**, four layers, fully routed,
...
## The two rows, and the margin outside them

**Rear row, left to right: `J2` line in, `J5` LOW, `J4` MID, `J3` HIGH,
`J1` power.** The three outputs are adjacent and in the panel's own order.
They used to be split by the power block (in, LOW, power, MID, HIGH),
which put mains-side wiring in the middle of the audio-side wiring and made
three amplifier looms fan out around it. Power now sits at the far end from
the input — the one signal with no gain in front of it.

**Nothing is outboard of a mounting screw.** The left and right margins are
wider than the front and rear ones (`EDGE_X`), sized so a hole fits beyond
the outermost part: `CLEAR + 2·MOUNT_R + MOUNT_EDGE_MIN`. That strip is
therefore empty by construction, so the corner holes cannot collide with
anything and only ever walk in Y — and `verify()` asserts the result rather
than trusting it.

It was not always so: the holes used to walk *inward* to clear the panel
row, which left `VR6` cantilevered 10.7 mm past the bottom-left screw. The
master volume is the control most likely to be leaned on.
...
* **On-board controls cost area, and they are what sets the width.** Six
  ...
  JLCPCB's 100 × 100 mm price tier**, at 147.3 mm on its long side — still a
