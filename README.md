...
`kicad/esp-p148-3way-crossover-retuned-quad-smd.kicad_sch` — the same
schematic as a **KiCad 8 schematic**, generated by `tools/gen_kicad.py`.
Open `kicad/crossover.kicad_pro`, or the `.kicad_sch` on its own.

It is **self-contained**: every symbol is embedded in the file's own
`lib_symbols`, one per component instance, so nothing needs to be
installed and no library path has to resolve. Nets are carried by labels
exactly as they are drawn.

It is not a second drawing of the circuit. `gen_kicad.py` reads the very
geometry `gen_schematic.py` lays out — every pin in absolute coordinates,
every wire polyline, every label — and translates it, so the two cannot
disagree about connectivity. And it checks: after emitting the file it
...
63.5 mm**, **four layers**, **fully routed**, nine front-panel controls on
...
```
board 149.6 x 63.5 mm | 86 footprints | 242 pads | 227 tracks | 159 vias
verified: all nets connected, all clearances >= 8 mil, no unrouted nets,
          pads match the schematic exactly
```

On the rear edge, left to right: **line input, LOW, MID, HIGH, power** —
the three outputs adjacent and in the same order as the front panel, with
the power block at the far end from the input so the two looms never run
together. On the front edge, left to right: MASTER,
