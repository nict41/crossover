...
> **The generator and the committed artifacts do not currently agree.**
> Everything in `pcb/` and `bom/` is still the previous *hand-placed*
> 106.7 × 55.9 mm board, which verifies clean. `tools/gen_pcb_smd.py` now
> produces a different, layout-first board that does **not** yet verify
> clean — the last seeds tried came back with 4–6 nets left in pieces — so
> the artifacts were deliberately left un-regenerated rather than replaced
> with a board that fails its own DRC. Re-run and re-commit `pcb/` only
> once a run verifies clean.

Where the routing failures are concentrated, for whoever picks this up:

* **Connections to the front-panel pot row** (`R7.1`→`VR1A`, `R17.1`→`VR2A`,
  `R8.1`→`VR1B`, and the `*_PRE` nets). The pot pads sit in the middle of a
  large courtyard and every one of them has to reach up into the board.
  `_side_counts()` in `place.py` decides which way a pad escapes by nearest
  courtyard edge, which classifies the outer pot pins as escaping
  *sideways* — where there is plenty of room and nothing to route to. That
  is a modelling error worth fixing before anything else.
* **Board size is found, not proven minimal** — it is whatever the search
  converged to, and a different `SEED` gives a different board. Spread
  between seeds is large (52–66 mm of height for the same inputs), so seed
  choice is currently doing more work than it should.
