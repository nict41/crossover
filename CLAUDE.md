...
The committed board is **97.5 × 82.3 mm** (`SEED=1 ROUTE_SEED=0`, 54
footprints), generated by the current pipeline. It verifies clean AND
passes `validate_fab.py` (EasyEDA import structure, JLCPCB fab limits,
BOM/CPL, LCSC stock, ground pour on both layers). It is larger than the
94.2 × 66.3 mm board it replaced because it now carries five capacitors
that board could not fit: `C11`-`C13` output DC blocking and `C9`/`C10`
bulk supply decoupling. Utilisation is 57% with a 6 × 60 mm empty strip,
so there is room to look for a tighter seed - only 1 candidate in 48
verified clean, so seed choice is still doing most of the work.
...
* **Only 1 candidate in 96 verifies clean** at the committed settings
  (seeds 1-72, route seeds 0-1). Seed choice is doing far more work than
  the search is. `tools/find_board.py` routes candidates in parallel and reports which verified — re-run it
